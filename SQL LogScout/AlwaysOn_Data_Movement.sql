...
BEGIN
    PRINT ''
    PRINT ' *** '
    PRINT ' *** Skipped creating and configuring event session AlwaysOn_Data_Movement. Principal ' + SUSER_SNAME() + ' does not have ALTER ANY EVENT SESSION permission.'
...
BEGIN
    RAISERROR ('SQL Server version is less than 2012. No additional data can be collected', 0, 1) WITH NOWAIT
    return
END

--Applies to: SQL Server 2014 SP2 (12.0.5000.0), SQL Server 2016 SP1 (13.0.4001), SQL Server 2017 RTM
IF (@servermajorversion = 13 and @serverbuild < 4001) or (@servermajorversion = 12 and @serverbuild < 5000)
BEGIN
    declare @ver_str varchar (128)
    set @ver_str = 'SQL Server version is ' + convert (varchar, @servermajorversion) + '.' + convert(varchar, @serverbuild)
    RAISERROR (@ver_str, 0, 1) WITH NOWAIT
    RAISERROR ('SQL Server version must be SQL Server 2014 SP2 (12.0.5000.0), SQL Server 2016 SP1 (13.0.4001), SQL Server 2017 RTM or later. No additional data can be collected', 0, 1) WITH NOWAIT
    return
END
ELSE
BEGIN
    IF HAS_PERMS_BY_NAME(NULL, NULL, 'ALTER ANY EVENT SESSION') = 1
    CREATE EVENT SESSION [AlwaysOn_Data_Movement] ON SERVER
