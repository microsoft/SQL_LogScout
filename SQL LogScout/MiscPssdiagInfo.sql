...
-- from this point on the script will access several Registry Keys and only SYSADMIN can do that
-- if user running this script is not SYSADMIN we will skip the and print a message
IF IS_SRVROLEMEMBER('sysadmin') <> 1
BEGIN -- non-sysadmin
	PRINT 'Server Principal ' + SUSER_SNAME() + ' is not a sysadmin.'
	PRINT 'Skipped obtaining information "operating system"'
	PRINT 'Skipped obtaining information "operating system install date"'
	PRINT 'Skipped obtaining information "operating system version build"'
	PRINT 'Skipped obtaining information "operating system version major"'
	PRINT 'Skipped obtaining information "operating system version minor"'
	PRINT 'Skipped obtaining information "registry ActivePowerScheme"'
	PRINT 'Skipped obtaining information "registry ActivePowerScheme (default)"'
	PRINT 'Skipped obtaining information "registry SystemManufacturer"'
	PRINT 'Skipped obtaining information "registry SystemProductName"'
END
ELSE
BEGIN -- SYSADMIN
...
insert into @startup select value_name Value, cast(value_data as nvarchar(max)) Data from sys.dm_server_registry where registry_key like '%\MSSQLServer\Parameters'
print ''
...
-- this DMV was introduced in SQL 2019
IF CAST(LEFT(CAST(SERVERPROPERTY('ProductVersion') AS NVARCHAR(30)),CHARINDEX('.', CAST(SERVERPROPERTY('ProductVersion') AS NVARCHAR(30)), 0)-1) AS INTEGER) >= 15
	select * From sys.dm_tran_persistent_version_store_stats
ELSE
	print 'Skipped sys.dm_tran_persistent_version_store_stats. SQL Server build version older than SQL Server 2019.'
print ''
go
/*
...
SELECT sess.NAME 'session_name', event_name,
CASE
