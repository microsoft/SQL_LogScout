...
BEGIN
	PRINT ''
	PRINT ' *** '
	PRINT ' *** Skipped creating and configuring event session xevent_general. Principal ' + SUSER_SNAME() + ' does not have ALTER ANY EVENT SESSION permission.'
...
IF HAS_PERMS_BY_NAME(NULL, NULL, 'ALTER ANY EVENT SESSION') = 1
    if exists (select * from sys.server_event_sessions where name= 'xevent_general') drop  EVENT SESSION [xevent_general] ON SERVER 
GO
IF HAS_PERMS_BY_NAME(NULL, NULL, 'ALTER ANY EVENT SESSION') = 1
    CREATE EVENT SESSION [xevent_general] ON SERVER 
