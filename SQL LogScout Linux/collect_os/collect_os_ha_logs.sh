...
#Starting the script
logger "Starting host HA logs collectors" "info_blue" "1" "1" "${sqllogscout_log:-/dev/null}" "${0##*/}"

#Execute only if pcs executable is installed
if [ -f /usr/sbin/pcs ];then
	logger "Collecting pcs status, Resource etc" "info" "1" "1" "${sqllogscout_log:-/dev/null}" "${0##*/}"
	...
	logger "Collecting pacemaker logs" "info" "1" "1" "${sqllogscout_log:-/dev/null}" "${0##*/}"
	...
	logger "looks pcs is not installed or not in a known path, skipping collecting host HA logs" "info" "1" "1" "${sqllogscout_log:-/dev/null}" "${0##*/}"
fi
